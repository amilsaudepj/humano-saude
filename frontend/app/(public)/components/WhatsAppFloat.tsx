...
export default function WhatsAppFloat() {
  ...
  const messages = [
    { text: 'Economize até 40%', icon: '💬' },
    { text: 'Análise em até 10 min', icon: '⚡' },
    { text: 'Plano personalizado', icon: '🎯' },
    { text: 'Especialista Online', icon: 'whatsapp' as const },
  ];
  ...
  return (
    <div
      className={`fixed left-[auto] right-4 bottom-4 sm:right-6 sm:bottom-6 z-50 transition-all duration-500 pb-[env(safe-area-inset-bottom)] pr-[env(safe-area-inset-right)] ${
    ...
    >
      ...
      <div className="absolute bottom-20 right-0 mb-2 animate-bounce">
        <div className="bg-white px-4 sm:px-6 py-3 rounded-2xl shadow-2xl border border-gray-200 max-w-[calc(100vw-6rem)]">
          <p className="text-sm font-bold text-gray-900 whitespace-nowrap flex items-center gap-2">
            {messages[messageIndex].icon === 'whatsapp' ? (
              <>
              ...
              <span>{`${messages[messageIndex].icon} ${messages[messageIndex].text}`}</span>
            )}
          </p>
        </div>
        {/* Arrow */}
