...
interface HeaderV2Props {
  /** Se true, exibe apenas logo e CTA WhatsApp (sem menu). Uso: completar-cotacao, etc. */
  minimal?: boolean;
}

export default function HeaderV2({ minimal = false }: HeaderV2Props) {
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-18 lg:h-20 flex items-center justify-between">
          ...
          {/* Nav Desktop (oculto em modo minimal) */}
          {!minimal && (
          ...
          {/* CTA WhatsApp só no desktop (mobile: só dentro do menu) + Hamburger quando não minimal */}
          <div className="flex items-center gap-3">
            {/* CTA no header: oculto em mobile/tablet (lg), visível só em desktop (xl+) para não duplicar no menu */}
            <a
              href="https://wa.me/5521988179407?text=Olá! Gostaria de uma cotação empresarial."
              target="_blank"
              rel="noopener noreferrer"
              className={`${minimal ? 'inline-flex' : 'hidden xl:inline-flex'} items-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all hover:-translate-y-0.5 bg-[#B8941F] hover:bg-[#C5A028] text-white`}
            >
            ...
            {/* Hamburger Mobile (apenas quando não minimal) */}
            {!minimal && (
              <button
      ...
      {/* ===== Menu Mobile Overlay (não exibir em modo minimal) ===== */}
      {!minimal && (
      <div
